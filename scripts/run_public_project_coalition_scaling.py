#!/usr/bin/env python3
"""Coalition-robust frontier scaling for public-project mechanisms.

This extension keeps the DSIC frontier exact and tests bounded coalition robustness
by progressively filtering only DSIC survivors. It avoids recomputing a full
frontier for every coalition size.
"""
...
def _survivors_by_cap(spec: PublicProjectSpec, max_coalition_size: int) -> dict[str, Any]:
    """
    Evaluate DSIC frontier then apply coalition checks incrementally.

    Returns a dictionary with cap-indexed serialized survivors, failure metadata,
    and DSIC-derived signature sets.
    """
    ...
    for cap in range(2, max_coalition_size + 1):
        ...
        for mechanism in survivors:
            verification = verify_public_project(mechanism, max_coalition_size=cap)
            if verification["accepted"]:
                next_survivors.append((mechanism, verification))
            else:
                if min_failing[mechanism.name] is None:
    ...
    # Any DSIC frontier rule that survives all tested caps keeps a None marker.
    for mechanism in survivors:
...
def main() -> None:
    ...
    artifact = {
        ...
        "question": (
            "How does bounded coalition robustness shrink the DSIC frontier "
            "for finite anonymous ternary public-project mechanisms as n and cost grow?"
        ),
    ...
    path = ROOT / "artifacts" / "public_project_coalition_scaling.json"
