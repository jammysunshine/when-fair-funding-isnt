#!/usr/bin/env python3
"""Independent verification for extended coalition frontier scaling artifact."""
...
def main() -> None:
    artifact_path = ROOT / "artifacts" / "public_project_coalition_scaling_extended.json"
    ...
    for block in artifact.get("by_n", []):
        ...
        for cost in configured_costs:
            ...
            min_failing_expected = {
                entry["name"]: entry["min_failing_coalition_size"]
            ...
            for cap in range(1, max_coalition_size + 1):
                ...
                expected_signature = sorted(row.get("survivor_signature_by_cap", {}).get(cap_key, []))
            ...
            for name, base_row in _rows_by_name(survivors_by_cap[cap_keys[0]]).items():
                ...
                for cap in range(1, int(mechanism["n_agents"]) + 1):
                    if not _check(cache, mechanism, cap).get("accepted", False):
                        ...
                        break
                recomputed_min_fail.append({"name": name, "min_failing_coalition_size": mechanism_observed_min_fail})
        ...
        selected_observed = [(n_agents, int(row.get("cost"))) for row in block.get("selected_cost_rows", [])]
    ...
    out = ROOT / "artifacts" / "public_project_coalition_scaling_extended_certificate.json"
